...
const MyBookings = () => {
  ...
  const handleDeleteBooking = async () => {
    ...
    try {
      const response = await axios.delete(`${apiUrl}delete-booking/${bookingToDelete}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto py-8 px-6">
        ...
        {bookings.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {bookings.map((booking) => (
              <div
                key={booking.id}
                className="relative cursor-pointer bg-white p-5 rounded-lg shadow-lg border border-gray-200 hover:shadow-xl transition transform hover:-translate-y-1"
              >
                {/* ❌ Delete Button */}
                <button
                ...
                <div className="flex items-center justify-between">
                  <h3 className="text-xl font-semibold text-blue-700">
                    {booking.origin} <FaPlaneDeparture className="inline-block ml-2" />
                  </h3>
                  <h3 className="text-xl font-semibold text-blue-700">
                    <FaPlaneArrival className="inline-block mr-2" /> {booking.destination}
                  </h3>
                </div>
                ...
                <div className="mt-4">
                  ...
                  <p className="flex items-center text-gray-700">
                    <FaUsers className="mr-2 text-blue-500" />
                    <strong>Passengers:</strong> {booking.passengersDetails.length}
                  </p>
                </div>
        ...
        {showDeleteModal && (
          <motion.div
            className="fixed inset-0 bg-gray-200 bg-opacity-30 backdrop-blur-md flex items-center justify-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
